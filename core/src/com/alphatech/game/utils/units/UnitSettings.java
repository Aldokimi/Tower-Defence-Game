...
public class UnitSettings {
    ...
    public UnitSettings(final Player bluePlayer, final Player redPlayer, final TowerSettings towerSettings) {
        ...
        // Animations
        idleRedSoldier1Animation = new Animation<TextureRegion>(0.08f, Textures.SOLDIER1_IDLE_RED.findRegions("idle"),
                PlayMode.LOOP);
        idleRedSoldier3Animation = new Animation<TextureRegion>(0.08f, Textures.SOLDIER3_IDLE_RED.findRegions("idle"),
                PlayMode.LOOP);
        ...
        idleBlueSoldier1Animation = new Animation<TextureRegion>(0.08f, Textures.SOLDIER1_IDLE_BLUE.findRegions("idle"),
                PlayMode.LOOP);
        idleBlueSoldier3Animation = new Animation<TextureRegion>(0.08f, Textures.SOLDIER3_IDLE_BLUE.findRegions("idle"),
                PlayMode.LOOP);
    ...
    public void endTurnRedPlayerSetUp(Player redPlayer, PathSettings pathSettings) {
        ...
        for (Unit unit : redPlayer.units) {
            if (unit instanceof NormalSoldier && unit.getCurrentState() == "IDLE"
                    && pathSettings.getChosenPath() != null) {

                unit.setPath(pathSettings.getChosenPath());
                unit.setState("WALK");
                unit.setAnimation(walkingRedSoldier1Animation);

                unit.setNextPathLevel(pathSettings.getPaths().get(unit.getPath()).size() - 2);
                unit.setPosition(
                        pathSettings.getPaths().get(Constants.PathNum.FIRST)
    ...
    public void endTurnBluePlayerSetUp(Player bluePlayer, PathSettings pathSettings) {
        ...
        for (Unit unit : bluePlayer.units) {
            if (unit instanceof NormalSoldier && unit.getCurrentState() == "IDLE"
                ...
                unit.setPosition(pathSettings.getPaths().get(Constants.PathNum.FIRST).get(0));
                                ...
                                if (pathSettings.getPaths().get(Constants.PathNum.FIRST).get(0)
                                    ...
                                    if (isACornerOfPath1ForRed(unit, pathSettings, elapsedTime, batch)) {
                                        ...
                                        if (unit.getIsXaxis()) {
                                            ...
                                            controlRedUnitsAnimationY(unit, false, pathSettings, batch, elapsedTime);
                                        }
                                    }
                                }
                                break;
                    ...
                    if (unit instanceof NormalSoldier) {
                        ...
                        switch (unit.getPath()) {
                            case FIRST:
                                ...
                                if (pathSettings.getPaths().get(Constants.PathNum.FIRST).get(4)
                                        .equals(unit.getPosition())) {
    ...
    private boolean isACornerOfPath1ForBlue(Unit unit, PathSettings pathSettings, Float elapsedTime,
        ...
        if (unit.getPosition().equals(pathSettings.getPaths().get(Constants.PathNum.FIRST).get(1))) {
            ...
            unit.setPosition(new Point(unit.getPosition().x + 1, unit.getPosition().y));
    ...
    private boolean isACornerOfPath1ForRed(Unit unit, PathSettings pathSettings, Float elapsedTime,
        ...
        if (unit.getPosition().equals(pathSettings.getPaths().get(Constants.PathNum.FIRST).get(3))) {
            ...
            unit.setPosition(new Point(unit.getPosition().x, unit.getPosition().y + 1));// Increasing the Y axis
    ...
    private void isACornerOfPath1XY(String Player, Unit unit, PathSettings pathSettings) {
        if (Player == "BLUE") {
            if ((pathSettings.getPaths().get(Constants.PathNum.FIRST).get(3)
                    .equals(new Point(unit.getPosition().x, unit.getPosition().y + 1))
            ...
            if (pathSettings.getPaths().get(Constants.PathNum.FIRST).get(2)
                    .equals(new Point(unit.getPosition().x - 1, unit.getPosition().y))) {
                unit.setIsXaxis(true);
    ...
    private void controlRedUnitsAnimationX(Unit unit, boolean negative, PathSettings pathSettings, SpriteBatch batch,
        ...
        if (!negative) {
            ...
            if (unit.getPosition().x < pathSettings.getPaths().get(Constants.PathNum.FIRST)
            ...
            if (unit.getPosition().x > pathSettings.getPaths().get(Constants.PathNum.FIRST)
                    .get(unit.getNextPathLevel()).x) {
                batch.draw(unit.getAnimation().getKeyFrame(elapsedTime, true), unit.getPosition().x,
                        pathSettings.getPaths().get(Constants.PathNum.FIRST).get(unit.getNextPathLevel()).y,
                        50, 1,
                        60,
                        45, 1, 1, 0);
                unit.setPosition(new Point(unit.getPosition().x - 1, unit.getPosition().y));
    ...
    private void controlRedUnitsAnimationY(Unit unit, boolean negative, PathSettings pathSettings, SpriteBatch batch,
        ...
        if (!negative) {
            ...
            if (unit.getPosition().y < pathSettings.getPaths().get(Constants.PathNum.FIRST)
                    .get(unit.getNextPathLevel()).y) {
                batch.draw(unit.getAnimation().getKeyFrame(elapsedTime, true),
                        pathSettings.getPaths().get(Constants.PathNum.FIRST).get(unit.getNextPathLevel()).x,
                        unit.getPosition().y,
                        50, 1,
                        60,
                        45, 1, 1, 0);
